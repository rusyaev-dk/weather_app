...
import 'package:weather_app/ui/resources/appbg.dart';
...
class WeatherProvider extends ChangeNotifier {
  ...
  //текущая иконка в зависимости от статуса погоды
  ...
  dynamic setDailyTemp(int index) {
    final dynamic dayTemp =
        ((_weatherData?.daily?[index].temp?.day ?? -kelvin) + kelvin).round();
    return dayTemp;
  }
  ...
  String setSunSet() {
    ...
    final setSunRise = DateTime.fromMillisecondsSinceEpoch(getSunTime * 1000);
    final String sunSet = DateFormat('HH:mm a').format(setSunRise);
    return sunSet;
  }
